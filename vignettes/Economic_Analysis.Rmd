...
## Example datasets
To illustrate the input data requirements and the practical application of youthu functions, we adapt a   [replication dataset from the youthu's sister package TTU](https://ready4-dev.github.io/TTU/articles/Replication_DS.html) to create synthetic (fake) single group and matched group datasets.  For our purposes we are going to use those records where a study participants had a baseline PHQ9 score of under 20.

```{r}
data("replication_popl_tb", package = "TTU")
seed_ds_tb <- replication_popl_tb %>% dplyr::filter(fkClientID %in% (replication_popl_tb %>% dplyr::filter(round=="Baseline" & PHQ9<20) %>% dplyr::pull(fkClientID)))
```
