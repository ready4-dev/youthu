...
add_qalys <- function(ds_tb,
 ...
 if(reshape_1L_lgl){
   ...
   updated_ds_tb <- updated_ds_tb %>% tidyr::pivot_wider(names_from = !!rlang::sym(round_var_nm_1L_chr),
                                                 values_from = vars_to_spread_chr)
 }
  return(updated_ds_tb)
}
make_ce_smry <- function(ds_tb,
                         indices,
                         benefits_pfx_chr = "qalys_dbl",
                         costs_pfx_1L_chr = "costs_dbl",
                         #iter_1L_int = 1L,
                         vars_to_incl_chr, # New
                         cmprsn_groups_chr = c("Intervention","Control"),
                         cmprsn_var_nm_1L_chr = "study_arm_chr",
                         benefits_var_nm_1L_chr = "qalys",
                         costs_var_nm_1L_chr = "costs"){
  costs_vars_chr <- paste0(costs_var_nm_1L_chr,"_",cmprsn_groups_chr)
benefits_vars_chr <- paste0(benefits_var_nm_1L_chr,"_",cmprsn_groups_chr)
new_nms_chr <- vars_to_incl_chr %>% purrr::map(~paste0(.x,"_",cmprsn_groups_chr)) # Pick up here
round_fup_1L_chr <- ds_smry_ls$round_lvls_chr[2]
selected_cols_chr <- c(costs_pfx_1L_chr, benefits_pfx_chr) %>% paste0("_",round_fup_1L_chr) 
summary_tb <- ds_tb[indices,] %>% 
  ...
  dplyr::select(dplyr::all_of(c(selected_cols_chr, cmprsn_var_nm_1L_chr))) %>%
    dplyr::rename_with(.cols = selected_cols_chr,
                       ~ifelse(startsWith(.x,costs_pfx_1L_chr),costs_var_nm_1L_chr,
                             ifelse(startsWith(.x,benefits_pfx_chr),benefits_var_nm_1L_chr,
                                    .x))) %>%
  dplyr::summarise(dplyr::across(.fns = mean)) %>%
  dplyr::ungroup()  %>%
  tidyr::pivot_wider(names_from = cmprsn_var_nm_1L_chr,
                      values_from = c(costs_var_nm_1L_chr, benefits_var_nm_1L_chr)) %>%
  dplyr::mutate(ICER = (`costs_Intervention` - `costs_Control`) / ## NEED TO CHANGE
                  (`qalys_Intervention` - `qalys_Control`)) %>%
...
make_he_smry <- function(ds_tb,
                         cmprsn_groups_chr = c("Intervention","Control"),
                         wtp_dbl =  50000){
  bootstraps_ls <- boot::boot(ds_tb, make_ce_smry, R=100)
...
```
```{r}
# econ_cmprsn_ds_tb <-econ_cmprsn_ds_tb %>%
#   add_qalys(utl_change_var_nm_1L_chr = paste0(ds_smry_ls$utl_var_nm_1L_chr,"_change_dbl"), 
#             utl_var_nm_1L_chr = ds_smry_ls$utl_var_nm_1L_chr,
#             duration_var_nm_1L_chr = "duration_prd",
#                       qalys_var_nm = "qalys_dbl")
...
```{r}
he_smry_ls <- econ_cmprsn_ds_tb %>% make_he_smry(cmprsn_groups_chr = c("Intervention","Control"),
                         wtp_dbl =  50000)
```
